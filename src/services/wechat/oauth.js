import wechatOAuth from '../../stores/wechatOAuth'
import _debug from 'debug'

const debug = _debug('backend:service:wechat:oauth')
/**
...
const getOpenid = async (code) => {
  return new Promise((resolve, reject) => {
    wechatOAuth.getAccessToken(code, (err, result) => {
      if (err) {
        debug('根据微信oauth回调的code换取accessToken错误: %s', err)
...
const getUserWechatProfile = async (openid) => {
  return new Promise((resolve, reject) => {
    wechatOAuth.getUser(openid, (err, result) => {
      if (err) {
        debug('根据用户微信的openid(%s)获取基本信息错误: %s', openid, err)
